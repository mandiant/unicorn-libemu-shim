...
// 
// 1.12.17 - David Zimmer
//    - added null checks after each lookup, 
//    - converted DYNLOAD_GETFUNC to function, 
//    - added lastDynLoadErr string error message      
...
bool uc_dyn_load(const char* path, int flags)
{
    ...
    if (g_dyn_handle == NULL) {
		...
		lastDynLoadErr = strdup("unicorn.dll");
        return false;
    }
